...
class DivVAE(nn.Module):
    def __init__(self, input_shape, latent_dim: int, encoder_depth: int = 2, decoder_depth: int = 2, delta: float = 0.05,
                 max_features: int = 3):
    ...
    def loss_function(self, q, origin, reconstruction, mean, log_var):
        # BCE = F.binary_cross_entropy(reconstruction, origin, reduction='sum')
        quantile_loss = torch.sum(self.quantile_loss(q, origin, reconstruction))
        # 0.5 * sum(1 + log(sigma^2) - mu^2 - sigma^2)
        KLD = -0.5 * torch.sum(1 + log_var - mean.pow(2) - log_var.exp())
        return quantile_loss + KLD
