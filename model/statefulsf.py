...
class Encoder(nn.Module):
    ...
    def init_state(self, x, batch_idx):
        queue = self.former_hidden_state
        if batch_idx == 0:
        ...
        elif self.cell_state is not None and self.cell_state.shape[1] != x.shape[0]:
            queue.append(queue.pop()[:, -x.shape[0]:, :])
            self.cell_state = self.cell_state[:, -x.shape[0]:, :]

    def forward(self, x, batch_idx):
        self.init_state(x, batch_idx)
...
class Decoder(nn.Module):
    ...
    def init_state(self, x, batch_size):
        queue = self.former_hidden_state
        ...
        elif cell_state[0].shape[1] > x.shape[0]:
            queue[0].append(queue[0].pop()[:, -x.shape[0]:, :])
            queue[1].append(queue[1].pop()[:, -x.shape[0]:, :])
            cell_state[0] = cell_state[0][:, -x.shape[0]:, :]
            cell_state[1] = cell_state[1][:, -x.shape[0]:, :]

    def forward(self, concatenate_z, batch_idx):
        self.init_state(concatenate_z, batch_idx)
        # 获取之前的状态
...
class StatefulSf(nn.Module):
    ...
    def training_step(self, input, batch_idx):
        input_hat, recon_loss = self.forward(input, batch_idx)
        return recon_loss
...
def evaluate(model, val_loader, n):
    outputs = [model.validation_step(to_device(batch, device), idx) for idx, [batch] in enumerate(val_loader)]
